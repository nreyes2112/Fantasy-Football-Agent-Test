...
- **Phase 1 exit checklist (§9): 6 of 8 items done.** The remaining 2 aren't code: (1) "daily ADP job verified 3+ consecutive days" needs real calendar time (the cron only recently went live); (2) "crosswalk covers 100% of the Charter universe, unmatched queue empty" needs Nick's human review of ~6 ambiguous FFC name-match proposals -- this project never auto-confirms a name match, so it's a decision only the owner can make. Not yet built: nflverse pbp/participation/nextgen-stats/full-rosters/draft-combine (nothing currently blocks on these; would close the data dictionary's last 5 metrics if prioritized).

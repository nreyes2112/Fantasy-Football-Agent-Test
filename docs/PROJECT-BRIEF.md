...
**Owner:** Nick | **Brief version:** 1.7 (2026-07-18) | **Update this header whenever status changes**
...
**Phase: 0 COMPLETE (charter approved 2026-07-18). Phase 1 build IN PROGRESS — nearly done.**
**Current step: Raw capture, the canonical-ID crosswalk (§3), full validation + GOLD marking (§6), the read-only agent access layer (§7), and nflverse player_stats + team_stats (2024-2025, weekly) are all built and (capture side) LIVE on GitHub Actions. `get_player_stats` AND `get_team_context` both read real data now (verified against Jahmyr Gibbs' actual 2025 stats and Detroit/Baltimore's real 2025 offensive profiles) instead of reporting unavailable. Only 1 of the original 8 access-layer functions remains an honest stub: `get_vacated_opportunity`, waiting on nflverse pbp/roster history to identify departed players. Note: within `get_team_context`, 3 specific sub-fields (PROE, Vegas win total, OL rank) are still reported unavailable even with team_stats pulled — none of the three are computable from any free source found; documented per-field, not silently dropped. `get_adp`, `get_depth_chart`, `get_comps`, `get_league_scoring`, `list_data_gaps` all verified against real data too. NEXT: (a) human review of the small FFC unmatched queue (~6 skill-position players); (b) nflverse pbp/roster history, unlocking the last access-layer stub; (c) the data dictionary (§5) — now largely meaningful since real stat data (target_share, air_yards_share, wopr, EPA, plays/game, pass rate) exists; (d) the curated layer beyond the crosswalk itself.**
...
| 1 Data platform | phase1-data-platform-design.md | ☐ In progress — raw capture, canonical-ID crosswalk (§3), full validation pipeline + GOLD marking (§6), the agent access layer (§7), and nflverse player_stats + team_stats (2024-2025) all built (`capture/`, `access/`) and (capture side) LIVE on GitHub Actions cron. ESPN settings-diff confirms charter §5 accurate. NOT yet: curated layer beyond the crosswalk, data dictionary (§5), nflverse pbp/roster history (1 of 8 access-layer functions still an honest stub pending this; 3 sub-fields of get_team_context are unavailable regardless -- no free source exists). |
...
9. Continue Phase 1 per design doc: nflverse pbp + rosters/depth-charts (unlocks `get_vacated_opportunity` — needs season-over-season departure detection), draft/combine measurables (would enrich `get_comps` beyond bio/draft-capital); data dictionary (§5) — now largely meaningful, but share metrics (carry_share, snap_share, route_participation) still need team-level plays/snap-count data (snap_counts hasn't been pulled); curated layer beyond the crosswalk — none of these exist yet

...
**Owner:** Nick | **Brief version:** 1.4 (2026-07-18) | **Update this header whenever status changes**
...
**Current step: Raw capture (Sleeper/FFC/ESPN), the canonical-ID crosswalk (§3), and the full staged validation pipeline + GOLD marking (§6) are all built and LIVE on GitHub Actions cron. `capture/validation.py` runs Stage 1 (schema completeness), Stage 2 (semantic: range checks, referential integrity, cross-source ADP agreement), and Stage 3 (statistical: day-over-day drift, gracefully degrading to "no prior day" on the first snapshot). First GOLD snapshot: 2026-07-18, 22/22 checks passed, verified both locally and on a real GitHub Actions run. Every threshold was calibrated from real measured data, not guessed — caught two bugs along the way: nflverse's `team` column uses a different abbreviation convention than Sleeper/ESPN (would have falsely flagged every legitimate team code), and cross-source ADP agreement needed to be round-sensitive (ESPN/FFC agree almost perfectly in the first ~50 picks but diverge naturally much more beyond pick 100 — real market noise, not error). GOLD is a data-quality gate; it does NOT require the crosswalk's FFC gaps to be resolved first (that's a separate, still-open exit-checklist item). NEXT: (a) human review of the small FFC unmatched queue (~6 skill-position players), (b) the curated layer beyond the crosswalk and the read-only agent access layer (§7), (c) nflverse stats/pbp/roster pulls (not touched yet -- only `load_ff_playerids()` is used so far), (d) the data dictionary (§5) with stability tags, which needs real stat data to be meaningful.**
...
| 1 Data platform | phase1-data-platform-design.md | ☐ In progress — raw capture, canonical-ID crosswalk (§3), and full validation pipeline + GOLD marking (§6) built (`capture/`) and LIVE on GitHub Actions cron. ESPN settings-diff confirms charter §5 accurate. NOT yet: curated layer beyond the crosswalk, agent access layer (§7), data dictionary (§5), nflverse stats/pbp/roster pulls. |
...
6. Continue Phase 1 per design doc: data dictionary (§5) — needs real stat data (target_share, etc.) to be meaningful, so likely follows the nflverse stats/pbp/roster pulls below; curated layer beyond the crosswalk; agent access layer (§7); nflverse stats/pbp/roster pulls (only `load_ff_playerids()` used so far) — none of these exist yet

...
**Owner:** Nick | **Brief version:** 1.9 (2026-07-18) | **Update this header whenever status changes**
...
**Current step: The data dictionary (§5) is done — [docs/data-dictionary.md](data-dictionary.md) covers all 18 required metrics, honestly scored: 10 fully computable and implemented (`access/metrics.py`), 2 partial, 6 genuinely deferred (5 need one more nflverse pull; `age_curve_position` is a methodology decision, not a data gap, deliberately left undefined rather than baking in an arbitrary curve). The flagship requirement — PPG under this league's exact scoring, not nflverse's own assumption — is implemented and **hand-verified against two independent real 2025 stat lines** (Josh Allen: hand calc 38.76 vs. code 38.76; Jahmyr Gibbs: hand calc 15.0 vs. code 15.0, specifically exercising the PPR path Allen's game didn't touch). `get_player_stats()` now exposes `fantasy_points_league_ppg`, `aDOT`, `EPA_per_target`, `TD_rate`, and `carry_share` as first-class metrics. NEXT: (a) human review of the small FFC unmatched queue (~6 skill-position players); (b) the curated layer beyond the crosswalk itself; (c) nflverse `load_snap_counts`/`load_participation` (unlocks snap_share/route_participation/YPRR) and `load_pbp` (unlocks success_rate/PROE) — the two items keeping the data dictionary from being 100% computable; none of these currently block anything else.**
...
| 1 Data platform | phase1-data-platform-design.md | ☐ Nearly done — raw capture, canonical-ID crosswalk (§3), full validation pipeline + GOLD marking (§6), ALL 8 agent access-layer functions (§7), and the data dictionary (§5) all built (`capture/`, `access/`, `docs/data-dictionary.md`) and (capture side) LIVE on GitHub Actions cron. ESPN settings-diff confirms charter §5 accurate. NOT yet: curated layer beyond the crosswalk, nflverse pbp/snap-counts/participation/full-rosters/draft-combine (not currently blocking anything). |
...
11. Continue Phase 1 per design doc: curated layer beyond the crosswalk; nflverse `load_snap_counts`/`load_participation` (unlocks snap_share/route_participation/YPRR) and `load_pbp` (unlocks success_rate/PROE, and closes the data dictionary's remaining gaps); full rosters/draft-combine — none of these are currently blocking anything, but round out Phase 1 per the original design

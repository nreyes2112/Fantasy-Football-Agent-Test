...
**Owner:** Nick | **Brief version:** 1.10 (2026-07-18) | **Update this header whenever status changes**
...
**Current step: nflverse snap_counts is pulled and `snap_share` is now computable (11 of 18 data-dictionary metrics implemented, up from 10). This table is keyed by `pfr_player_id`, not gsis_id like player_stats/team_stats — resolved by reusing the same `resolve_source()` crosswalk machinery Sleeper/ESPN already go through, rather than inventing a new matching mechanism. Verified against real data: Josh Allen (full-time starting QB) shows 98.11% snap share; Jahmyr Gibbs shows 67%, sensibly higher than his 54.98% carry_share since he's also used on passing downs. Only 5 data-dictionary metrics remain genuinely deferred (route_participation, YPRR, success_rate, PROE — all need `load_participation`/`load_pbp`, bigger lifts than this one; `age_curve_position` is a methodology decision, not a data gap). NEXT: (a) human review of the small FFC unmatched queue (~6 skill-position players); (b) the curated layer beyond the crosswalk itself; (c) `load_participation`/`load_nextgen_stats` and `load_pbp` if closing the remaining data-dictionary gaps becomes a priority — none of this currently blocks anything.**
...
| 1 Data platform | phase1-data-platform-design.md | ☐ Nearly done — raw capture, canonical-ID crosswalk (§3) including snap_counts resolution, full validation pipeline + GOLD marking (§6), ALL 8 agent access-layer functions (§7), and the data dictionary (§5, 11/18 computable) all built (`capture/`, `access/`, `docs/data-dictionary.md`) and (capture side) LIVE on GitHub Actions cron. ESPN settings-diff confirms charter §5 accurate. NOT yet: curated layer beyond the crosswalk, nflverse pbp/participation/full-rosters/draft-combine (not currently blocking anything). |
...
12. Continue Phase 1 per design doc: curated layer beyond the crosswalk; nflverse `load_participation`/`load_nextgen_stats` (unlocks route_participation/YPRR) and `load_pbp` (unlocks success_rate/PROE, closing the data dictionary's remaining 5 gaps); full rosters/draft-combine — none of these are currently blocking anything, but round out Phase 1 per the original design

...
**Owner:** Nick | **Brief version:** 1.12 (2026-07-18) | **Update this header whenever status changes**
...
**Phase: 0 COMPLETE (charter approved 2026-07-18). Phase 1 build essentially complete -- 7 of 8 exit-checklist items done.**
**Current step: Nick reviewed the FFC unmatched queue and confirmed 6 players. 4 were genuine name collisions with a different real player sharing the same normalized name (Justin Jefferson WR vs. a rookie LB; Lamar Jackson QB vs. a CB; DJ Moore WR vs. a CB free agent; Antonio Williams WR vs. an RB free agent) -- confirmed which one FFC/ESPN actually meant. 2 were nickname/formal-name mismatches (Kenny Gainwell = Kenneth Gainwell; Chig Okonkwo = Chigoziem Okonkwo). These are now persisted in `capture/ffc_confirmed_matches.json` (git-tracked, human-maintained -- never auto-generated) and applied every crosswalk rebuild. Result: charter-universe coverage improved (QB 24/24 and WR 60/60 now fully resolved, up from 23/24 and 58/60; RB 46/48, up from 45/48) and **the unmatched queue now has zero skill-position players** -- only team defenses/kickers remain, which are permanently out of charter scope (§4) and were never going to resolve anyway. Verified locally and on GitHub Actions (GOLD still marks clean, 24/24 checks).**
**Phase 1 exit checklist (§9): 7 of 8 items now DONE.** The only remaining item is genuinely time-based, not a decision or code gap: "daily ADP snapshot job verified for 3+ consecutive days" -- only one calendar day (2026-07-18, ET) of real snapshot history exists so far; the cron needs to keep running across real days to accumulate a 3-day streak. (Note: a recent Actions run's commit message read "2026-07-19" because its `date -u` timestamp had crossed UTC midnight, but the actual snapshot data is correctly still under `2026-07-18` -- the capture code uses ET, not UTC, specifically to avoid a premature rollover; the commit message text is cosmetically misleading, the data isn't.)
**NEXT: no more Phase 1 code depends on anything outstanding. Options: (a) start Phase 2 (backtest harness); (b) close the data dictionary's last 5 metrics (route_participation/YPRR need `load_participation`; success_rate/PROE need `load_pbp`); (c) just let the cron keep running to accumulate the 3-day streak and formally close Phase 1's checklist.**

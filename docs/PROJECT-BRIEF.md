...
**Owner:** Nick | **Brief version:** 1.6 (2026-07-18) | **Update this header whenever status changes**
...
**Current step: Raw capture, the canonical-ID crosswalk (§3), full validation + GOLD marking (§6), the read-only agent access layer (§7), and now nflverse player_stats (2024-2025, weekly) are all built and (capture side) LIVE on GitHub Actions. `get_player_stats` reads real data now (supports season/last4/last8 windows, verified against Jahmyr Gibbs' actual 2025 stats) instead of reporting unavailable. Only 2 of the original 8 access-layer functions remain honest stubs: `get_team_context` and `get_vacated_opportunity`, both waiting on nflverse schedules/Vegas win totals/pbp, which haven't been pulled. `get_adp`, `get_depth_chart`, `get_comps`, `get_league_scoring`, `list_data_gaps` all verified against real data. NEXT: (a) human review of the small FFC unmatched queue (~6 skill-position players); (b) nflverse schedules/Vegas win totals + rosters/depth-charts + pbp, unlocking the last 2 access-layer stubs; (c) the data dictionary (§5) — now partially meaningful since real stat data (target_share, air_yards_share, wopr, EPA) exists; (d) the curated layer beyond the crosswalk itself.**
...
| 1 Data platform | phase1-data-platform-design.md | ☐ In progress — raw capture, canonical-ID crosswalk (§3), full validation pipeline + GOLD marking (§6), the agent access layer (§7), and nflverse player_stats (2024-2025) all built (`capture/`, `access/`) and (capture side) LIVE on GitHub Actions cron. ESPN settings-diff confirms charter §5 accurate. NOT yet: curated layer beyond the crosswalk, data dictionary (§5), nflverse schedules/win-totals/rosters/pbp (2 of 8 access-layer functions still honest stubs pending this). |
...
8. Continue Phase 1 per design doc: nflverse schedules/Vegas win totals (unlocks `get_team_context`), rosters/depth charts, pbp (unlocks `get_vacated_opportunity`), draft/combine measurables; data dictionary (§5) — now partially meaningful since real stat data exists, but shares metrics (carry_share, snap_share, route_participation) still need team-level plays/snap-count data not pulled; curated layer beyond the crosswalk — none of these exist yet
